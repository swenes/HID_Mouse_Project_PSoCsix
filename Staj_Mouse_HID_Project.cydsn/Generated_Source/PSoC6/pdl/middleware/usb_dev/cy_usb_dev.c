...
static void SetDeviceAddress(cy_stc_usb_dev_context_t *context);
...
* The states \ref CY_USB_DEV_EP_IDLE and \ref CY_USB_DEV_EP_COMPLETED 
* are not handled by this function and return. 
*
* \return
* Status code of the function execution \ref cy_en_usb_dev_status_t.
*
...
* After abort request the function waits for 1 millisecond to give to the USB 
* Host complete on-going transfer to this endpoint. Therefore, this function 
* returns call \ref Cy_USB_Dev_ReadEpNonBlocking to get received data (if any).
*
* \param endpoint
                                                  ...
                                                  cy_stc_usb_dev_context_t *context)
{
    ...
    epState = Cy_USBFS_Dev_Drv_Abort(context->drvBase, endpoint, context->drvContext);

    if (CY_USB_DEV_EP_PENDING == epState)
    {
        /* Wait 1 ms to complete possible on-going transfer */
        (void) context->handleTimeout(1);

        epState = Cy_USBFS_Dev_Drv_GetEndpointState(context->drvBase, endpoint, context->drvContext);

        if (CY_USB_DEV_EP_PENDING == epState)
        {
            /* Abort operation applied: clean-up endpoint */
            retStatus = (cy_en_usb_dev_status_t) 
                            Cy_USBFS_Dev_Drv_AbortComplete(context->drvBase, endpoint, context->drvContext);
    ...
    else if (CY_USB_DEV_EP_IDLE == epState)
    {
        /* Special case: Abort called when endpoint is setup for a transfer */
        retStatus = CY_USB_DEV_SUCCESS;
    }
    else
    {
...
static void BusResetCallback(USBFS_Type *base, struct cy_stc_usbfs_dev_drv_context *drvContext)
{
    ...
    while (NULL != curItem)
    {
        if (NULL != curItem->class->busReset)
        {
            /* Execute callback */
            curItem->class->busReset(curItem->classData, context);
        }
            ...
            if (transfer->setup.wLength > 0U)
            {
                ...
                if (CY_USB_DEV_DIR_DEVICE_TO_HOST == transfer->direction)
                {
                    ...
                    if (transfer->setup.wLength > transfer->remaining)
                    {
                        ...
                        transfer->zlp = (0U == (transfer->remaining % CY_USB_DEV_EP0_PACKET_SIZE));
                    }
                    
                    /* DATA stage (IN direction) */
                ...
                else
                {
                    ...
                    /* DATA stage (OUT direction) */
                    uint8_t dummyByte = 0U;
                    (void) Cy_USBFS_Dev_Drv_Ep0Read(context->drvBase, &dummyByte, 0U, context->drvContext);
                }
            }
            else
            {
                /* STATUS stage (IN direction) */
...
****************************************************************************//**
*
* Sets device address and changes internal state to CY_USB_DEV_ADDRESSED.
*
* \param context
* The pointer to the context structure \ref cy_stc_usb_dev_context_t allocated
* by the user. The structure is used during the USB Device operation for internal
...
static void SetDeviceAddress(cy_stc_usb_dev_context_t *context)
{
    if (context->state == CY_USB_DEV_ADDRESS)
    {
        Cy_USBFS_Dev_Drv_SetDeviceAddress(context->drvBase, context->address);

        /* Set device state depends on address value */
        context->state = (0U == context->address) ?
                            CY_USB_DEV_DEFAULT : CY_USB_DEV_ADDRESSED;
    }
}
...
static cy_en_usb_dev_status_t HandleIn(cy_stc_usb_dev_context_t *context)
{
    ...
    if (CY_USB_DEV_DIR_DEVICE_TO_HOST == transfer->direction)
    {
        ...
        if (0U == transfer->remaining)
        {
            if (transfer->zlp)
            {
                ...
                uint8_t dummyByte = 0U;
                (void) Cy_USBFS_Dev_Drv_Ep0Write(context->drvBase, &dummyByte, 0U, context->drvContext);
                transfer->zlp = false;
            }
            else
            {
        ...
        {
            /* Write to endpoint 0 */
            uint16_t packetSize = (uint16_t ) Cy_USBFS_Dev_Drv_Ep0Write(context->drvBase, transfer->ptr, 
    ...
    {
        /* STATUS stage (direction IN) completed: STALL IN and OUT */

        /* Exception: set address after STATUS stage completed */
...
static cy_en_usb_dev_status_t HandleOut(cy_stc_usb_dev_context_t *context)
{
    ...
    if (CY_USB_DEV_DIR_HOST_TO_DEVICE == transfer->direction)
    {
        ...
        uint16_t packetSize = (uint16_t) Cy_USBFS_Dev_Drv_Ep0Read(context->drvBase, transfer->ptr, 
                                                                  (uint32_t) transfer->remaining, 
                                                                  context->drvContext);
    ...
    {
        /* STATUS stage (direction OUT) completed: STALL IN and OUT */
    }
...
static void Ep0SetupCallback(USBFS_Type *base, struct cy_stc_usbfs_dev_drv_context *drvContext)
{
    ...
    {
        /* Protocol stall */
        Cy_USBFS_Dev_Drv_Ep0Stall(base, drvContext);
    }
}
...
static void Ep0InCallback(USBFS_Type *base, struct cy_stc_usbfs_dev_drv_context *drvContext)
{
    ...
    /* Endpoint 0 IN packet received event: always returns CY_USB_DEV_SUCCESS */
    (void) HandleIn(context);
}
                                               ...
                                               cy_stc_usb_dev_context_t const *context)
{
    ...
    switch (transfer->setup.bmRequestType.recipient)
    {
        ...
        case CY_USB_DEV_RECIPIENT_ENDPOINT:
        {
            ...
            cy_en_usb_dev_ep_state_t epState = Cy_USBFS_Dev_Drv_GetEndpointStallState(context->drvBase, endpoint, context->drvContext);
                                                ...
                                                cy_stc_usb_dev_context_t *context)
{
    ...
    context->address = CY_LO8(transfer->setup.wValue);

    /* Switch state to set address after STATUS stage in SetDeviceAddress */
    context->state = CY_USB_DEV_ADDRESS;
                                                      ...
                                                      cy_stc_usb_dev_context_t *context)
{
    ...
    else
    {
        ...
        if (config <= context->devDescriptors->numConfigurations)
        {
            ...
            /* Configure Device and endpoints */
            ...
            if (CY_USB_DEV_SUCCESS == retStatus)
            {
                ...
                if (CY_USB_DEV_SUCCESS == retStatus)
                {
                    ...
                    while (NULL != curItem)
                    {
                        if (NULL != curItem->class->setConfiguration)
                        {
                            /* Execute callback */
                            retStatus = curItem->class->setConfiguration(config, curItem->classData, context);
            ...
            if (CY_USB_DEV_SUCCESS == retStatus)
            {
                ...
                Cy_USBFS_Dev_Drv_ConfigDeviceComplete(context->drvBase, context->drvContext);
                                        ...
                                        cy_stc_usb_dev_context_t *context)
{
    ...
    {
        /* Execute callback */
        if (NULL != curItem->class->setInterface)
        {
            retStatus = curItem->class->setInterface(interface, alternate, 
                                                  ...
                                                  cy_stc_usb_dev_context_t          *context)
{
    ...
    {
        /* Execute callback */
        if (NULL != curItem->class->requestReceived)
        {
            retStatus = curItem->class->requestReceived(transfer, curItem->classData, context);
                                                           ...
                                                           cy_stc_usb_dev_context_t          *context)
{
    ...
    {
        /* Execute callback */
        if (NULL != curItem->class->requestCompleted)
        {
            retStatus = curItem->class->requestCompleted(transfer, curItem->classData, context);
...
cy_en_usb_dev_status_t Cy_USB_Dev_RegisterClass(cy_stc_usb_dev_class_ll_item_t *classItem,
                                                cy_stc_usb_dev_class_t *class,
                                                void *classContext,
                                                cy_stc_usb_dev_context_t *context)
{
    if ((NULL == classItem) || (NULL == class))
    {
        return CY_USB_DEV_BAD_PARAM;
    }
